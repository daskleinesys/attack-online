...
class SQLCommands {
	...
	private static function LoadUserQueries() {
		...
		// user
			...
			self::$DataSource->load_query('get_all_user_data',"SELECT id, name, lastname, login, email, status, verify FROM user WHERE id = :id_user");
	...
	private static function LoadGameSpecificQueries($id_game) {
		...
		// moves
			...
			$steps_table = "z" . $id_game . "_moves_steps";
			$steps_area_table = "z" . $id_game . "_moves_steps_zareas";
			$unit_moves_table = "z" . $id_game . "_moves_units";
			// select start moves
			self::$DataSource->load_query('get_start_move',"SELECT moves.id, moves.id_user, moves.id_phase, moves.round, moves.deleted, steps.step, step_areas.id_zarea
				FROM $moves_table as moves
				LEFT JOIN $steps_table as steps ON (moves.id = steps.id_zmove)
				LEFT JOIN $steps_area_table as step_areas ON (steps.id = step_areas.id_zmove_step)
				WHERE moves.id = :id_move AND moves.id_phase = :id_phase AND moves.round = :round",true);
			self::$DataSource->load_query('get_start_move_for_user',"SELECT moves.id FROM $moves_table AS moves WHERE moves.id_user = :id_user AND id_phase = :id_phase AND round = :round",true);
			// select land moves
			self::$DataSource->load_query('get_land_move',"SELECT moves.id,moves.id_user,moves.id_phase,moves.round,moves.deleted,steps.step,areas.id_zarea,units.id_unit,units.numberof
				FROM $moves_table AS moves
				LEFT JOIN $steps_table AS steps ON (moves.id=steps.id_zmove)
				LEFT JOIN $steps_area_table AS areas ON (steps.id=areas.id_zmove_step)
				LEFT JOIN $unit_moves_table AS units ON (moves.id=units.id_zmove)
			...
			self::$DataSource->load_query('get_specific_moves',"SELECT id FROM $moves_table WHERE id_phase LIKE :id_phase AND round LIKE :round AND id_user LIKE :id_user AND deleted = 0",true);
			self::$DataSource->load_query('create_move',"INSERT INTO $moves_table (id_user,id_phase,round) VALUES (:id_user,:id_phase,:round)",true);
			self::$DataSource->load_query('create_step_for_move',"INSERT INTO $steps_table (id_zmove,step) VALUES (:id_move,:step)",true);
			self::$DataSource->load_query('get_id_for_step_and_move',"SELECT id FROM $steps_table WHERE id_zmove = :id_move AND step = :step",true);
			self::$DataSource->load_query('insert_move_area_for_step',"INSERT INTO $steps_area_table (id_zmove_step,id_zarea) VALUES (:id_step,:id_zarea)",true);
			self::$DataSource->load_query('insert_land_units_for_move',"INSERT INTO $unit_moves_table (id_unit,id_zmove,numberof) VALUES (:id_unit,:id_move,:count)",true);
			self::$DataSource->load_query('delete_move_areas_for_step',"DELETE FROM $steps_area_table WHERE id_zmove_step IN (SELECT id FROM $steps_table WHERE id_zmove = :id_move AND step = :step)",true);
			self::$DataSource->load_query('delete_move_areas_for_move',"DELETE FROM $steps_area_table WHERE id_zmove_step IN (SELECT id FROM $steps_table WHERE id_zmove = :id_move)",true);
			self::$DataSource->load_query('delete_steps_for_move',"DELETE FROM $steps_table WHERE id_zmove = :id_move",true);
			self::$DataSource->load_query('delete_units_for_move',"DELETE FROM $unit_moves_table WHERE id_zmove = :id_move",true);
