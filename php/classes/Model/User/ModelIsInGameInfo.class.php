...
class ModelIsInGameInfo {
	...
	public static function getIsInGameInfo($id_user, $id_game) {
		if (isset(self::$models[$id_user][$id_game])) return self::$models[$id_user][$id_game];
	...
	public static function joinGame($id_user,$id_game,$id_color = null) {
		...
		if ($id_color != null) $id_color = intval($id_color);
		...
		while ($iter->hasNext()) {
			if ($iter->next() == $_User) throw new JoinUserException('User allready in this game.');
		}
		
		// check if color is free
		if ($id_color != null) {
			if (!$_Game->checkIfColorIsFree($id_color)) $id_color = null;
		}
		// get first free color
		if ($id_color == null) {
			$color_iter = ModelColor::iterator();
			while ($color_iter->hasNext()) {
				$_Color = $color_iter->next();
				if (!$_Game->checkIfColorIsFree($_Color->getId())) continue;
				$id_color = $_Color->getId();
				break;
			}
		}
		if ($id_color == null) throw new JoinUserException('No free color found.');
	 ...
	 * @return void
	 */
	public static function deleteIsInGameInfos($id_game,$id_user = null) {
		$query = ($id_user == null) ? 'delete_iig_info_for_game' : 'delete_iig_info_for_user';
		$dict = array(':id_game' => intval($id_game));
		if ($id_user != null) $dict[':id_user'] = intval($id_user);
	 ...
	 * @return void
	 */
	public static function leaveGame($id_game,$id_user) {
	...
	public static function isUserInGame($id_user,$id_game) {
		if (isset(self::$models[$id_user][$id_game])) return true;
		try {
	...
	public function setColor($id_color) {
		...
		if (!ModelGame::getGame($this->id_game)->checkIfColorIsFree()) throw new GameAdministrationException('Color already taken!');
	...
	private function fill_member_vars() {
		...
		if (empty($result)) throw new NullPointerException('User is not in this game!');
