...
class ModelInGamePhaseInfo {
	...
	public function setNotificationRule($id_phase,$rule) {
		...
		if (!isset($this->notification_rules[$id_phase])) throw new NullPointerException('No such rule found.');
	...
	public function setIsReady($id_phase,$is_ready) {
		$id_phase = intval($id_phase);
		if (!isset($this->is_ready[$id_phase])) throw new NullPointerException('No such info found.');
	...
	private function fill_member_vars() {
		...
		while ($iter->hasNext()) {
			...
			if (empty($result)) $this->create_info($id_phase);
			else {
