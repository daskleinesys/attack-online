...
class ModelStartRegion {
	...
	public static function getRegionsForSet($id_set) {
		...
		if (isset(self::$regions[$id_set])) return self::$regions[$id_set];
		
		$query = 'get_startregions_for_set';
		$dict[':id_set'] = $id_set;
		$result = DataSource::getInstance()->epp($query,$dict);
		if (empty($result)) throw new NullPointerException('Set not found.');
	...
	public static function getRegionsForSetAndOption($id_set,$option_number) {
		...
		if (!isset(self::$regions[$id_set])) self::getRegionsForSet($id_set);
		foreach (self::$regions[$id_set] as $options_types) {
			if (isset($options_types[$option_number])) return $options_types[$option_number];
		}
