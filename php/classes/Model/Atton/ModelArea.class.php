...
class ModelArea {
	...
	private function __construct($id_area) {
		...
		if (!$this->fill_member_vars()) throw new NullPointerException('Area not found.');
	}
	...
	public static function getArea($id_area) {
		if (isset(self::$areas[$id_area])) return self::$areas[$id_area];
	...
	public function getAdjecents() {
		if (!empty($this->adjecents)) return $this->adjecents;
