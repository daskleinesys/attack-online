...
class ModelInGameLandUnit {
	private static $units = array(); // array(int id_game => array(int id_zarea => array(int id_user => array(int id_unit => ModelInGameLandUnit))))
	...
	public static function getModelByIdZAreaUserUnit($id_game,$id_zarea,$id_user,$id_unit) {
		...
		if (isset(self::$units[$id_game][$id_zarea][$id_user][$id_unit])) return self::$units[$id_game][$id_zarea][$id_user][$id_unit];
	...
	/**
	 * returns all models for the given area/user, does NOT create new ones if not all units are present
	 * @param $id_game int
	 * @param $id_zarea int
	 * @param $id_user int
	...
	public function setCount($count) {
		...
		$dict = array(':id_zunit'=>$this->id,':count'=>intval($count));
	...
	public function addCount($count) {
		$this->setCount($this->count + intval($count));
	}
